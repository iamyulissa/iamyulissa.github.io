...
async function handleChatImageUpload(event) {
    ...
    try {
        ...
        // 生成唯一的图片ID - 使用兼容的generateUUID()提高健壮性
        const imageId = `chat_img_${generateUUID()}`;
...
async function sendImageMessage(imageId, fileId, fileName) {
    ...
    // 创建图片消息对象 - 使用兼容的generateUUID()生成更健壮的ID
    const imageMessage = {
...
function openBubbleDesigner() {
    try {
        // 检查当前是否为单文件模式
        // 简单方法：检查当前页面是否为 file:// 协议
        const isFileProtocol = window.location.protocol === 'file:';
        
        if (isFileProtocol) {
            console.log('🔄 检测到 file:// 协议，使用 iframe 模式');
            openBubbleInModal();
        } else {
            // 尝试打开独立窗口
            ...
            if (!bubbleWindow) {
                console.log('⚠️ 弹窗被阻止，使用 iframe 模式');
                openBubbleInModal();
                return;
            }
            ...
            setTimeout(() => {
                try {
                    // 如果窗口关闭了或者没有正确加载，回退到iframe
                    if (bubbleWindow.closed) {
                        console.log('🔄 窗口已关闭，使用 iframe 模式');
                        openBubbleInModal();
                    } else {
                        console.log('✅ 多文件模式：bubble.html 已在新窗口中打开');
                        bubbleWindow.focus();
                    }
                } catch (error) {
                    console.log('🔄 窗口检测失败，可能是跨域问题，使用 iframe 模式');
                    try {
                        bubbleWindow.close();
                    } catch (e) {
                        // 忽略关闭窗口时的错误
        ...
        console.log('🔄 发生错误，回退到 iframe 方式');
        openBubbleInModal();
    }
}

/**
 * 在iframe中打开气泡设计器（单文件模式）
 */
function openBubbleInModal() {
    // 获取隐藏的气泡设计器iframe
    const hiddenIframe = document.getElementById('bubbleDesignerFrame');
    if (!hiddenIframe) {
        console.error('❌ 未找到气泡设计器iframe');
        showToast('气泡设计器加载失败');
        return;
    }
    ...
    titleBar.innerHTML = `
        <span>🎨 气泡设计器</span>
        <button id="closeBubbleDesigner" style="
            background: none;
            border: none;
            font-size: 18px;
            cursor: pointer;
    ...
    // 克隆隐藏的iframe并显示
    const iframe = hiddenIframe.cloneNode(true);
    iframe.id = 'bubbleDesignerModalFrame';
    ...
    // 如果隐藏的iframe还没有加载完成，等待加载
    if (!hiddenIframe.contentDocument || hiddenIframe.contentDocument.readyState !== 'complete') {
        console.log('🔄 等待气泡设计器iframe加载...');
        hiddenIframe.onload = function() {
            console.log('✅ 气泡设计器iframe加载完成，克隆内容');
            // 重新克隆已加载的iframe
            const newIframe = hiddenIframe.cloneNode(true);
            newIframe.id = 'bubbleDesignerModalFrame';
    ...
    // 设置关闭事件
    document.getElementById('closeBubbleDesigner').onclick = function() {
    ...
    console.log('🎨 气泡设计器已在模态iframe中打开（完整版本）');
}
...
/**
 * 检测用户访问环境
 * @returns {boolean} true表示网页访问，false表示本地访问
 */
 ...
 * 关键通知管理器
 */
const criticalNotificationManager = {
    STORAGE_KEY: 'critical-notification-read-20240920',
    ...
    showCriticalNotification() {
        // 检查是否已存在通知
        if (document.getElementById('criticalNotification')) {
            return;
        }

        // 创建通知容器
        const notification = document.createElement('div');
        notification.id = 'criticalNotification';
        notification.className = 'critical-notification';

        // 通知内容
        notification.innerHTML = `
            <div class="critical-notification-overlay"></div>
            <div class="critical-notification-content">
                <div class="critical-notification-icon">⚠️</div>
                <div class="critical-notification-title">重要通知</div>
                <div class="critical-notification-message">
                    <p><strong>网页版将于9月20日晚9点停止服务！</strong></p>
                    <p>我们转成了文件版！请及时备份数据，前往交流群的群公告和群相册，查看数据迁移教程，以免影响正常使用！！</p>
                </div>
                <button class="critical-notification-button" onclick="criticalNotificationManager.closeNotification()">
                    我知道了
                </button>
            </div>
        `;
...
function checkCriticalNotification() {
    // 只有网页访问且通知未读时才显示
    if (isWebAccess() && !criticalNotificationManager.isNotificationRead()) {
        criticalNotificationManager.showCriticalNotification();
    }
}
