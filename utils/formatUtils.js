...
/**
 * 生成兼容的UUID格式字符串
 * 兼容crypto.randomUUID()的回退方案，适用于单文件构建和非HTTPS环境
 * @returns {string} UUID格式的字符串 (如: "a1b2c3d4-e5f6-7890-abcd-ef1234567890")
 */
function generateUUID() {
    ...
    if (typeof crypto !== 'undefined' && crypto.randomUUID) {
        try {
            ...
            console.warn('crypto.randomUUID()不可用，使用回退方案:', error);
        }
    }
    ...
    // 构造标准UUID格式: xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx
    const uuid = [
        timestamp.substr(0, 8),
        random1.substr(0, 4),
        '4' + random1.substr(4, 3), // 版本号4
        ((parseInt(random2.charAt(0), 16) & 0x3) | 0x8).toString(16) + random2.substr(1, 3), // 变体位
        random2.substr(4, 4) + random3.substr(0, 8)
    ].join('-');
